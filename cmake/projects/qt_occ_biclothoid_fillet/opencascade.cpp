...
void Opencascade::remove_all(){
    ...
    //! You have to do more here to remove all.
}
...
void Opencascade::m_initialize_context()
{
    ...
    toolcone=draw_primitives().draw_3d_tcp_cone({0,0,0},0,0.08,0.2);
    toolcone=draw_primitives().rotate_3d(toolcone,{0,0,0},0,0.5*M_PI,0);
    // toolcone=draw_primitives().colorize(toolcone,Quantity_NOC_WHITE,0);
    show_shape(toolcone);
}
